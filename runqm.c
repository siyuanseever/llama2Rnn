...
typedef struct {
    ...
    int mem_len; // memory sequende length
} Config;
...
typedef struct {
    ...
    float* rms_mem_weight; // (layer, dim)
    float* rms_mff_weight; // (layer, dim)
    ...
    // weight for memory
    float *ori_mem; // (layer, mem_len, dim)
    QuantizedTensor *aw1; // (layer, hidden_dim, dim)
    QuantizedTensor *aw2; // (layer, dim, hidden_dim)
    QuantizedTensor *aw3; // (layer, hidden_dim, dim)
    // final rmsnorm
...
typedef struct {
    ...
    float* mem_cache; // (layer, mem_len, dim)
} RunState;
...
void malloc_run_state(RunState* s, Config* p) {
    ...
    s->mem_cache = calloc(p->n_layers * p->mem_len * p->dim, sizeof(float));
...
void memory_map_weights(TransformerWeights *w, Config* p, void* ptr, uint8_t shared_classifier) {
    ...
    fptr += p->n_layers * p->dim;
    w->rms_mem_weight = fptr;
    fptr += p->n_layers * p->dim;
    w->rms_mff_weight = fptr;
    fptr += p->n_layers * p->dim;
    w->ori_mem = fptr;
    fptr += p->n_layers * p->mem_len * p->dim;
    ...
    // w->ori_mem = init_quantized_tensors(&ptr, p->n_layers, p->mem_len * p->dim);
    w->aw1 = init_quantized_tensors(&ptr, p->n_layers, p->dim * p->hidden_dim);
    w->aw2 = init_quantized_tensors(&ptr, p->n_layers, p->hidden_dim * p->dim);
    w->aw3 = init_quantized_tensors(&ptr, p->n_layers, p->dim * p->hidden_dim);
...
void update_memory_cache(Transformer* transformer, float* memory, int mem_pos, int l) {
    ...
    int mem_len = p->mem_len;

    // save memory at this time step (pos) to our memory
    int mloff = l * p->mem_len * dim; // memory layer offset for convenience
    float* mem_row = memory + mloff + mem_pos * dim;
    memcpy(x, mem_row, dim * sizeof(float));

    // ffn rmsnorm
    rmsnorm(s->xb, x, w->rms_mff_weight + l*dim, dim);

    // Now for FFN in PyTorch we have: self.w2(F.silu(self.w1(x)) * self.w3(x))
    // first calculate self.w1(x) and self.w3(x)
    quantize(&s->xq, s->xb, dim);
    matmul(s->hb, &s->xq, w->aw1 + l, dim, hidden_dim);
    matmul(s->hb2, &s->xq, w->aw3 + l, dim, hidden_dim);
    ...
    matmul(s->xb, &s->hq, w->aw2 + l, hidden_dim, dim);
    ...
    // attention rmsnorm
    rmsnorm(s->xb, x, w->rms_mem_weight + l*dim, dim);
    ...
    // save key,value at this memory step (mem_pos) to our kv cache
    int loff = l * p->seq_len * kv_dim; // kv cache layer offset for convenience
    float* key_cache_row = s->key_cache + loff + mem_pos * kv_dim;
    float* value_cache_row = s->value_cache + loff + mem_pos * kv_dim;
