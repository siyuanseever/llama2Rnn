...
def download():
    ...
    data_dir = os.path.join(DATA_CACHE_DIR, DATA_NAME_DIR)
...
def train_vocab(vocab_size):
    ...
    shard_filenames = []
    for data_name in TRAIN_DATA_DIRS: 
        train_data_dir = os.path.join(DATA_CACHE_DIR, data_name)
        shard_filenames.extend(sorted(glob.glob(os.path.join(train_data_dir, "*.json"))))
...
def process_shard(args, vocab_size):
    ...
    if vocab_size == 0:
        ...
        bin_dir = os.path.join(DATA_CACHE_DIR, DATA_NAME_DIR, f"tok{vocab_size}")
...
class PretokDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        if self.vocab_source == "llama2":
            ...
            bin_dir = os.path.join(DATA_CACHE_DIR, DATA_NAME_DIR)
            shard_filenames = sorted(glob.glob(os.path.join(bin_dir, "*.bin")))
        elif self.vocab_source == "custom":
            # the .bin files are in tok{N} directory
            bin_dir = os.path.join(DATA_CACHE_DIR, DATA_NAME_DIR, f"tok{self.vocab_size}")
            shard_filenames = sorted(glob.glob(os.path.join(bin_dir, "*.bin")))
