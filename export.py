...
def version2_export_mem(model, filepath, group_size=64):
    """
    Export the model weights in Q8_0 into .bin file to be read from C.
    That is:
    - quantize all weights to symmetric int8, in range [-127, 127]
    ...
    weights = [
        ...
        *[layer.feed_forward.w3.weight for layer in model.layers],
        # mem
        # // *[layer.attention.origin_mem for layer in model.layers],
        *[layer.attention.feed_forward.w1.weight for layer in model.layers],
        *[layer.attention.feed_forward.w2.weight for layer in model.layers],
        *[layer.attention.feed_forward.w3.weight for layer in model.layers],
    ]
    shared_classifier = torch.equal(model.tok_embeddings.weight, model.output.weight)
    if not shared_classifier:
        weights.append(model.output.weight)
    ...
    header = struct.pack('iiiiiiii', p.dim, hidden_dim, p.n_layers, p.n_heads,
                                    n_kv_heads, p.vocab_size, p.max_seq_len, p.memseqlen)
    out_file.write(header)
    # 4) write some other flags
    out_file.write(struct.pack('B', int(shared_classifier)))
    out_file.write(struct.pack('i', group_size)) # group size used for quantization
    ...
    for layer in model.layers: # MLP norms
        serialize_fp32(out_file, layer.ffn_norm.weight)
    for layer in model.layers: # memory norms
        serialize_fp32(out_file, layer.attention.memory_norm.weight)
    for layer in model.layers: # memory MLP norms
        serialize_fp32(out_file, layer.attention.ffn_norm.weight)
    for layer in model.layers: # memory MLP norms
        serialize_fp32(out_file, layer.attention.origin_mem)
    serialize_fp32(out_file, model.norm.weight) # final pre-classifier norm
...
def model_export_mem(model, filepath, version, dtype=torch.float32):
    """
    Versions docs:
    v-1:huggingface export, i.e. intended for use outside of this repo, in HF
    v0: legacy llama2.c float format, DEPRECATED
    ...
    """
    if version == 2:
        version2_export_mem(model, filepath)
    else:
        raise ValueError(f"unknown version {version}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mem", action="store_true", help="if use mem model")
    ...
    # export
    if args.mem:
        model_export_mem(model, args.filepath, args.version, args.dtype)
    else:
